...
class CompilerThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.finished_compilation.emit(
                False,
                "PyInstaller не найден. Установите пакет и убедитесь, что он в PATH.",
            )
        except Exception as exc:
            self.finished_compilation.emit(False, f'Критическая ошибка: {exc}')
...
class PyCompilerApp(QMainWindow):
    ...
    def _setup_window(self):
        self.setWindowTitle('Gemini Voice Assistant - Compiler')
    ...
    def _create_widgets(self):
        ...
        spec_label = QLabel('Spec-файл для компиляции:')
        self.spec_combo = QComboBox()
        self.spec_combo.setEditable(True)
        self.spec_combo.setPlaceholderText('D:/path/to/Gemini_Voice_Assistant.spec')
        ...
        self.build_btn = QPushButton('Скомпилировать')
    ...
    def _browse_spec_file(self):
        start_dir = os.path.dirname(self.spec_combo.currentText().strip()) or '.'
        file_path, _ = QFileDialog.getOpenFileName(
            self, 'Выберите spec-файл', start_dir, 'Spec Files (*.spec)'
        )
        if file_path:
    ...
    def _start_compilation(self):
        ...
        if not spec_file or not os.path.exists(spec_file):
            QMessageBox.warning(self, 'Ошибка', 'Укажите корректный путь к .spec файлу.')
            return
        ...
        self.statusBar().showMessage('Компиляция…')
    ...
    def _run_post_build(self, spec_file):
        ...
        self._append_log('>>> post_build.py найден. Запуск…')
        try:
    ...
    if sys.platform == 'win32':
        try:
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
                'gemini.voice.assistant.compiler'
            )
        except Exception:
            pass
