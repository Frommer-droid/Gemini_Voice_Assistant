...
from everything_search import EverythingSearchHandler
from window_snap import snap_to_screen
...
class ModernWindow(QMainWindow):
    ...
    def create_gemini_tab(self):
        ...
        self.gemini_prompt_combo.setSizeAdjustPolicy(
        ...
        for btn in (
            self.prompt_add_btn,
            self.prompt_rename_btn,
            self.prompt_delete_btn,
        ):
        ...
        self.gemini_prompt_combo.currentTextChanged.connect(
            self.on_gemini_prompt_profile_changed
        )
        ...
        self.gemini_prompt_edit.textChanged.connect(
            self.on_gemini_prompt_text_changed_profile
        )
    ...
    def create_main_tab(self):
        ...
        g3_level = (
            self.assistant.settings.get("gemini3_thinking_level") or "high"
        ).lower()
        ...
        g25_mode = (
            self.assistant.settings.get("gemini25_flash_mode") or "thinking"
        ).lower()
        ...
        self.hold_win_shift = QRadioButton("Левый Win + левый Shift (по умолчанию)")
        self.hold_ctrl_shift = QRadioButton("Левый Ctrl + левый Shift")
        current_hold = (
            self.assistant.settings.get("hold_hotkey") or "win+shift"
        ).lower()
        self.hold_win_shift.setChecked(current_hold == "win+shift")
        self.hold_ctrl_shift.setChecked(current_hold == "ctrl+shift")
        for btn in [
            self.hold_win_shift,
            self.hold_ctrl_shift,
        ]:
    ...
    def create_audio_tab(self):
        ...
        self.no_speech_spin.setValue(self.assistant.settings.get("no_speech_threshold"))
    ...
    def create_vpn_tab(self):
        ...
        self.vless_port_spin.setValue(self.assistant.settings.get("vless_port", 10809))
    ...
    def vless_connect(self):
        if not self.vless_enabled_check.isChecked():
            self.vless_enabled_check.setChecked(True)
            self.assistant.save_setting("vless_enabled", True)
    ...
    def vless_test(self):
        port = self.assistant.vless_manager.local_socks_port
        if self.assistant.vless_manager.is_running and self.assistant.settings.get(
            "vless_enabled", False
        ):
    ...
    def on_vless_enabled_changed(self, state):
        ...
        if not enabled and self.assistant.vless_manager.is_running:
            self.assistant.vless_manager.stop()
            self.update_vpn_status()
            self.assistant.setup_gemini()
    ...
    def on_vless_port_changed(self, value):
        ...
        manager = self.assistant.vless_manager
        ...
        if was_running:
            url = self.assistant.settings.get("vless_url", "").strip()
            if url:
                manager.start(url)
                self.update_vpn_status()
                self.assistant.setup_gemini()
    ...
    def _connect_signals(self):
        ...
        self.assistant.ui_signals.request_show_logs.connect(self.open_log_viewer)
        ...
        self.hold_win_shift.toggled.connect(
            lambda checked: checked and self.on_hold_hotkey_changed("win+shift")
        )
        self.hold_ctrl_shift.toggled.connect(
            lambda checked: checked and self.on_hold_hotkey_changed("ctrl+shift")
        )
    ...
    def on_gemini_prompt_text_changed_profile(self):
        ...
        prompts = self.assistant.settings.get("gemini_prompts", {})
        if hasattr(self, "gemini_prompt_combo") and isinstance(
            self.gemini_prompt_combo, QComboBox
        ):
    ...
    def on_add_gemini_prompt_profile(self):
        ...
        if name in prompts:
            QMessageBox.warning(
                self, "Ошибка", "Профиль с таким именем уже существует."
            )
            return
        prompts[name] = ""
        self.assistant.save_setting("gemini_prompts", prompts)
    ...
    def on_rename_gemini_prompt_profile(self):
        ...
        if not current:
            return
        text, ok = QInputDialog.getText(
            self, "Переименовать профиль", "Новое имя:", text=current
        )
        new_name = text.strip()
    ...
    def on_delete_gemini_prompt_profile(self):
        ...
        if len(prompts) <= 1:
            QMessageBox.information(
                self, "Информация", "Нельзя удалить единственный профиль."
            )
            return
        current = self.gemini_prompt_combo.currentText()
    ...
    def on_model_changed(self):
        ...
        self.assistant.show_status(f"Активация {model_name}...", COLORS["accent"], True)
    ...
    def _set_g3_level(self, level):
        ...
        self.assistant.show_status(
            f"Gemini 3.0 thinking: {level}", COLORS["accent"], False
        )
    ...
    def _set_g25_mode(self, mode):
        ...
        self.assistant.show_status(
            f"Gemini 2.5 Flash: {status}", COLORS["accent"], False
        )
    ...
    def on_hold_hotkey_changed(self, value):
        normalized = self.assistant._normalize_hold_hotkey(value)
        self.assistant.save_setting("hold_hotkey", normalized)
        self.assistant.update_hotkey_combo(normalized)
        self.assistant.show_status(
            f"Удержание: {normalized}", COLORS["accent"], False
        )
...
class VoiceAssistant:
    ...
    def _apply_settings_migrations(self, settings):
        ...
        normalized_hold = self._normalize_hold_hotkey(current_hold)
    ...
    def _normalize_hold_hotkey(self, name: Optional[str]) -> str:
    ...
    def update_hotkey_combo(self, name: Optional[str] = None):
        ...
        name = self._normalize_hold_hotkey(
    ...
    def _update_cached_settings(self):
        ...
        self.hold_hotkey = self._normalize_hold_hotkey(
        ...
        log_message(
            f"Кэшированные настройки обновлены: win_shift='{self.win_shift_mode}', f1='{self.f1_mode}', hold_hotkey='{self.hold_hotkey}', combo={self.normalized_hotkey_combo}"
        )
        ...
        try:
            ...
            if httpx is not None:
                try:
                    ...
                    log_message(
                        f"HTTP клиент Gemini настроен: http2=off, retries=3, connect_timeout=10s, proxy={proxy_display}"
                    )
                except Exception as e:
                    log_message(f"⚠ Не удалось настроить кастомный HTTP клиент: {e}")
    ...
    def _update_hold_recording_state(self):
        ...
        if active_hotkey:
            if not self.is_recording and not self.is_continuous_recording:
                log_message(
                    f"Горячая клавиша удержания сработала: {self.normalized_hotkey_combo}"
                )
    ...
    def on_press(self, key):
        ...
        with self.keys_lock:
            ...
            if comparable_key in self.normalized_hotkey_combo and len(self.pressed_keys) != before_size:
                log_message(
                    f"Нажата клавиша хоткея: {comparable_key}, текущее множество: {self.pressed_keys}"
                )
        ...
        with self.keys_lock:
            ...
            if comparable_key in self.pressed_keys:
                ...
                if comparable_key in self.normalized_hotkey_combo:
                    log_message(
                        f"Отпущена клавиша хоткея: {comparable_key}, текущее множество: {self.pressed_keys}"
                    )
            self._update_hold_recording_state()
    ...
    def start_recording(self, continuous=False, source=None):
        ...
        self._recording_hotkey_source = source
    ...
    def _should_skip_silence(self, audio_samples, chunk_peak):
        ...
        min_duration = max(0.1, self.settings.get("silence_duration_ms", 600) / 1000.0)
    ...
    def _handle_launch_command(self, text):
        ...
        try:
            log_message(f"COMMAND EXECUTED: {command} (admin={admin_requested})")
    ...
    def _open_path_safely(self, path):
        ...
        try:
            ...
            log_message(f"Ошибка открытия пути '{path}': {e}")
            self.show_status(
                "Не удалось открыть найденное", COLORS["btn_warning"], False
            )

    def _handle_everything_search(self, text):
        """
        ...
        """
        try:
            handled, paths = self.search_handler.handle_voice_command(
                text=text,
                client=self.client,
                status_cb=self.show_status,
            ...
            log_message(f"Ошибка обработки поиска через Everything: {e}")
            log_message(traceback.format_exc())
            self.show_status("Ошибка поиска Everything", COLORS["btn_warning"], False)
            return False
    ...
    def _process_audio_whisper(self, audio_np, is_final_segment=False):
        ...
        try:
            ...
            if final_text:
                ...
                # Проверка голосового поиска через Everything
                if self._handle_everything_search(final_text):
                    return
            # -----------------------------------------------------------------------
            ...
            if final_text:
                ...
                pro_word = (self.settings.get("pro_word", "про") or "про").strip().lower()
                flash_word = (
                    (self.settings.get("flash_word", "флеш") or "флеш")
                    .strip()
                    .lower()
                )
                selection_word = (
        ...
        while True:
            ...
            try:
                ...
                if self._is_transient_network_error(e) and retries_for_model < max_retries:
                    ...
                    log_message(
                        f"{display_name}: сетевой сбой ({e}). Повтор {retries_for_model}/{max_retries} через {delay:.1f}с"
                    )
                    time.sleep(delay)
                    continue
    ...
    def _finalize_task_output(
        ...
        with self._task_lock:
            if task_id != self._current_task_id or self._task_finalized:
                return False
            insert = (
                self._current_task_insert_text if insert_text is None else insert_text
            )
